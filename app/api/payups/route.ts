...
// =============================================================================
// API Route Handler: POST /api/payups
// Creates a new payup (temporary payment intent)
// =============================================================================
...
export async function POST(request: NextRequest) {
  ...
  return Exit.match(result, {
    onFailure: (cause) => {
      ...
      if (error._tag === "Some" && error.value && typeof error.value === "object" && "_tag" in error.value) {
    ...
    onSuccess: (output) => {
      const result = output as { payupId: string; paymentUrl: string; expiresAt: Date };
      return NextResponse.json(
        {
          payupId: result.payupId,

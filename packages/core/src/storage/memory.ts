...
import { createStorageLayer, type StorageProvider } from "./provider";
...
export const createMemoryStorage = (options?: { prefix?: string }): StorageProvider => {
  ...
  return {
    get: <T>(key: string) => Effect.sync(() => getEntry<T>(key)?.value ?? null),
    ...
    exists: (key: string) => Effect.sync(() => getEntry(key) !== null),

    incr: (key: string) =>
      Effect.sync(() => {
        const entry = getEntry<number>(key);
        const newValue = (entry?.value ?? 0) + 1;
        store.set(prefixKey(key), { value: newValue, expiresAt: entry?.expiresAt ?? null });
        return newValue;
      }),
    ...
    hget: <T>(key: string, field: string) =>
      Effect.sync(() => {
        ...
        return hash ? (hash.get(field) as T) ?? null : null;
      }),
    ...
    hdel: (key: string, field: string) =>
      Effect.sync(() => {
        hashStore.get(prefixKey(key))?.delete(field);
      }),
    ...
    hgetall: <T>(key: string) =>
      Effect.sync(() => {
        ...
        hash.forEach((v, k) => { result[k] = v as T; });
        return result;
      }),
    ...
    rpop: <T>(key: string) =>
      Effect.sync(() => {
        ...
        return list?.length ? (list.pop() as T) : null;
      }),
    ...
    llen: (key: string) => Effect.sync(() => listStore.get(prefixKey(key))?.length ?? 0),
...
export const MemoryStorageLive = (options?: { prefix?: string }) =>
  createStorageLayer(createMemoryStorage(options));

export { Storage, createStorageLayer, type StorageProvider } from "./provider";
